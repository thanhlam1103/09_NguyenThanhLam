...
var getApiAccountParam = {
  ...
  departmentName: "",
  getParamString() {
    return "?pageNumber=" + this.pageNumber + "&search=" + this.search + "&role=" + this.role + "&departmentName=" + this.departmentName;
  },
};
...
function paginationAccount(result) {
  ...
  $("#pagination-account").html(
    `<a title="Previous" href = "javascript:void(0)" onclick="toAccountPage(${activeNumber > 1 ? activeNumber - 1 : activeNumber})">Previous</a>`
  );
  ...
  $("#pagination-account").append(
    `<a href = "javascript:void(0)" title="Next" onclick="toAccountPage(${activeNumber < result.totalPages ? activeNumber + 1 : activeNumber})">Next</a>`
  );
}
...
function initAccountDepartmentList() {
  ...
  $.ajax({
    ...
    // thêm .content vì nó đang ở dạng page
      result.content.forEach(function (item) {
        $("#account-department-filter").append('<option value="' + item.id + '">' + item.name + "</option>");
        $("#account-department-select").append('<option value="' + item.id + '">' + item.name+ "</option>");
      });
    },
  });
}
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      list.forEach(function (item) {
        $("#department-select").append('<option value="' + item.id + '">' + item.name + "</option>");
      });
    },
  });
}
