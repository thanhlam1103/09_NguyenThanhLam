...
DROP VIEW SaleEmployee;
CREATE VIEW SaleEmployee AS
SELECT  *
FROM (SELECT A.*,d.DepartmentName
FROM `Account` A
JOIN Department d ON A.DepartmentID = d.DepartmentID
WHERE d.DepartmentName = 'Sale') AS Sale;



-- Question 2: Tạo view có chứa thông tin các account tham gia vào nhiều group nhất
-- Cách 1
DROP VIEW AccountJoinGroups;
CREATE VIEW AccountJoinGroups AS
WITH AccountGroupCounts AS (
    SELECT A.AccountID, COUNT(GroupID) AS GroupCount
    FROM `Account` A
...
SELECT A.*
FROM `Account` A
JOIN AccountGroupCounts agc ON A.AccountID = agc.AccountID
WHERE agc.GroupCount = (
		SELECT MAX(GroupCount) FROM AccountGroupCounts
	);

SELECT  *
FROM AccountJoinGroups;

-- Question 3: Tạo view có chứa câu hỏi có những content quá dài (content quá 300 từ được coi là quá dài) và xóa nó đi
DROP VIEW LongContentQuestions;
...
WHERE QuestionID IN (
    ...
    WHERE LENGTH(Content) > 12 ) AS DeleteContent
);
SELECT *
FROM LongContentQuestions;

-- Question 4: Tạo view có chứa danh sách các phòng ban có nhiều nhân viên nhất
DROP VIEW DepartmentMuchEmployees;
CREATE VIEW DepartmentMuchEmployees AS
SELECT DPM.DepartmentID, DPM.DepartmentName, COUNT(A.AccountID) AS Num_Account
FROM Department DPM
JOIN `Account` A ON DPM.DepartmentID = A.DepartmentID
...
SELECT *
FROM DepartmentMuchEmployees;

-- Question 5: Tạo view có chứa tất các các câu hỏi do user họ Nguyễn tạo
DROP VIEW NguyenUserQuestions;
CREATE VIEW NguyenUserQuestions AS
SELECT *
	FROM `Question` 
	WHERE UserName IN  (
    SELECT UserName
